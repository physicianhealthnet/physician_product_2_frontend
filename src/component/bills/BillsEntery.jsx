...
function BillsEntery({ outerswaper }) {
  ...
  useEffect(() => {
    if (sittingData.price) {
      setSittingData((prev) => ({
        ...prev,
        total: Number(prev.price),
      }));
    }
  }, [sittingData.price]);
  ...
  const handleSubmmit = async () => {
    try {
      ...
      let pdfBase64 = null;
      try {
        const pdfDataUri = await html2pdf().from(element).set(options).output('datauristring');
      ...
      {!outerswaper && (
        ...
        >
          <Icon
            ...
            className={`${swaper ? "rotate-45" : "rotate-0"
        ...
        {!swaper ? (
          <div className="w-full">
            {bills.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="text-lg font-bold text-slate-800 ">
                        Bill ID: <span className="text-primary-600 ">{bill.treatmentBillId}</span>
                      </h3>
                      <button
                        onClick={async (e) => {
                          e.stopPropagation();
                      ...
                      >
                        <Icon icon="material-symbols:share-outline" width="18" height="18" />
                      </button>
                    </div>
                    <div className="flex flex-col gap-1 text-slate-600 ">
                      <p className="flex justify-between">
                        <span>Date:</span>
                        <span className="font-medium text-slate-800 ">{new Date(bill.invoiceDate).toLocaleDateString("en-IN")}</span>
                      </p>
                      <p className="flex justify-between">
                        <span>Total Amount:</span>
                        <span className="font-bold text-green-600 ">₹{bill.totalAmount}</span>
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center justify-center py-12 text-slate-500 ">
                <Icon icon="material-symbols:receipt-long-outline" width={48} className="mb-2 opacity-50" />
          ...
          <div className="flex flex-col gap-6 w-full lg:w-3/4">
            ...
            <div className="bg-slate-50  p-6 shadow-sm border border-slate-200  rounded-xl flex flex-col gap-4">
              <h3 className="font-bold text-lg text-slate-800  mb-2">Bill Details</h3>
            ...
            <div className="bg-white  border border-slate-200  shadow-sm rounded-xl p-6">
              <h3 className="font-bold text-lg text-slate-800  mb-4">Items / Treatments</h3>
              <div className="flex flex-col md:flex-row flex-wrap items-end gap-4 mb-6">
                {[
                  ...
                  { label: "Notes", name: "notes", value: sittingData.notes, width: "flex-[2]" },
                ].map((data, index) => (
                  <div key={index} className={`w-full md:w-auto ${data.width || ''}`}>
                    <Input
                      label={data.label}
                      type={data.type || "text"}
                  ...
                  <tbody className="divide-y divide-slate-200 ">
                    {finalSittingData.map((data, index) => (
                      <tr key={index} className="hover:bg-slate-50 :bg-slate-800/50 transition-colors">
                        ...
                        <td className="p-3 text-slate-800  font-medium">{data.total}</td>
                        <td className="p-3 text-slate-600  text-sm max-w-[200px] truncate">{data.notes}</td>
                        <td className="p-3">
        ...
        <div className="w-screen h-screen fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-h-[95vh] max-w-[900px] flex flex-col bg-white  rounded-xl shadow-2xl overflow-hidden">
            <div className="flex flex-row items-center justify-between p-4 border-b border-slate-200  bg-slate-50 ">
              <h1 className="text-xl m-0 font-bold text-slate-800 ">Bill Preview</h1>
              <div className="flex items-center gap-2">
                ...
                {billOptionSwaper && (
                  <>
                    <Button
                      onClick={() => {
                        ...
                        const options = {
                          margin: 0,
                          with: 850,
                          filename: `${selectedBill?.treatmentBillId || "invoice"
                            }.pdf`,
                          image: { type: "jpeg", quality: 0.98 },
                          html2canvas: { scale: 2 },
                          jsPDF: {
                    ...
                    <button
                      className={`p-2 rounded-lg bg-orange-500 hover:bg-orange-600 text-white transition-colors ${outerswaper ? "hidden" : null
                        }`}
                      onClick={() => {
                    ...
                    {master?.userType === "master" && (
                      <button
                        className={`p-2 rounded-lg bg-red-500 hover:bg-red-600 text-white transition-colors ${outerswaper ? "hidden" : null
                          }`}
